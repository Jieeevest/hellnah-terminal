...
import { BookOpen, Clock, BarChart2, LayoutGrid, Square, Plus, Scan, LogOut, ShieldAlert, User, Ticket, HelpCircle, Newspaper, Bell, Radio, Zap, Play, TrendingUp, Scale } from 'lucide-react'
...
const handleMarketTypeChange = (mt: MarketType) => {
          ...
          <div className={cn("absolute inset-0 flex flex-col bg-background", mobileView === 'panel' ? 'z-10' : 'hidden')}>
            <div className="flex overflow-x-auto scrollbar-none border-b border-border shrink-0">
              {([
                ...
                { id: 'trades',    icon: <Clock className="h-3 w-3" />,    label: 'Trade' },
                { id: 'signal',    icon: <BarChart2 className="h-3 w-3" />,label: 'Analisa' },
                { id: 'scanner',   icon: <Scan className="h-3 w-3" />,     label: 'Scan' },
                { id: 'heatmap',   icon: <LayoutGrid className="h-3 w-3" />,label: 'Map' },
                { id: 'volume',    icon: <Zap className="h-3 w-3" />,      label: 'Anomali' },
                { id: 'funding',   icon: <TrendingUp className="h-3 w-3" />,label: 'Fund' },
                { id: 'coinglass', icon: <Scale className="h-3 w-3" />,    label: 'OI' },
                { id: 'news',      icon: <Newspaper className="h-3 w-3" />,label: 'Berita' },
                { id: 'alert',     icon: <Bell className="h-3 w-3" />,     label: 'Alert' },
                { id: 'social',    icon: <Radio className="h-3 w-3" />,    label: 'Sosial' },
                { id: 'backtest',  icon: <Play className="h-3 w-3" />,     label: 'Backtest' },
  ...
  { id: 'heatmap',   icon: <LayoutGrid className="h-3 w-3" />,      label: 'Heatmap' },
  { id: 'volume',    icon: <Zap className="h-3 w-3" />,             label: 'Anomali' },
  { id: 'funding',   icon: <TrendingUp className="h-3 w-3" />,      label: 'Funding' },
  { id: 'coinglass', icon: <Scale className="h-3 w-3" />,           label: 'OI' },
  { id: 'news',      icon: <Newspaper className="h-3 w-3" />,       label: 'Berita' },
  { id: 'alert',     icon: <Bell className="h-3 w-3" />,            label: 'Alert' },
  { id: 'social',    icon: <Radio className="h-3 w-3" />,           label: 'Sosial' },
