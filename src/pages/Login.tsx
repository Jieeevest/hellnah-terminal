...
import { Activity, TrendingUp, ShieldCheck, Mail, Lock, User, AlertCircle, ArrowRight, Ticket } from 'lucide-react'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = isLogin 
        ? { email, password } 
        : { username, email, password, ref: refCode }
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="w-full max-w-[420px]">
          ...
          <div className="bg-black/60 backdrop-blur-2xl border border-white/10 p-8 sm:p-10 rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] relative overflow-hidden group">
            ...
            <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
              ...
              {!isLogin && (
                <div className="space-y-2">
                  <label className="text-xs font-bold tracking-wider text-muted-foreground ml-1">KODE REFERRAL (OPSIONAL)</label>
                  <div className="relative group/input">
                    <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none text-muted-foreground group-focus-within/input:text-primary transition-colors">
                    ...
                    <input 
                      ...
                      className="w-full bg-[#0a0a0a] border border-white/10 text-white text-sm rounded-2xl pl-12 pr-4 py-3.5 outline-none focus:border-primary/50 focus:bg-[#111] focus:ring-4 focus:ring-primary/10 transition-all placeholder:text-white/20 uppercase"
                      placeholder="KODE123"
                    />
                  </div>
                </div>
